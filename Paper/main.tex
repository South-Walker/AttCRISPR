...
\end{equation}\\
 ...
 which means that the first-order preference of the $i$-th position $\widetilde{A_i}$ can be calculated as
\begin{equation}
\widetilde{A}_i=\sum_j^lB_{ij}A_i\label{eq:06}\vspace*{-10pt}
\end{equation}\\
 Suppose that in Equation \ref{eq:06} the first-order original preference at each position is relatively independent and weight matrix B is independent of any first-order original preference, 
...
\centerline{\includegraphics[scale=0.4]{category.png}}
...
Equation \ref{eq:01} is the inventory flow formula.
Text
...
\begin{figure}[!tpb]%figure2
    \centerline{\includegraphics[scale=0.5]{category.png}}
    \caption{The overview of AttCRISPR. 
    It consists of two components, namely CNNs and RNNs. 
    The CNN component is used to capture spatial nucleotide preference, 
    while the RNN component is used to capture temporal nucleotide preference. 
    Finally, a weighted average method with trainable weight is used to predict activity.}\label{fig:02}
\end{figure}
...
\begin{equation}
\sum \text{\it x}+ \text{\it y} =\text{\it Z}\label{eq:0}\vspace*{-10pt}
\end{equation}
...
{\ldots}{\ldots} text follows.
\begin{equation}
    \partial y = \sum^{len}_ia_i\partial x_i\label{eq:02}\vspace*{-10pt}
\end{equation}
...
\end{minipage}}
%\centerline{\includegraphics[scale=0.6]{category.png}}
